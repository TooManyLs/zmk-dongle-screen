...
struct layer_roller_state { uint8_t index; };

static void layer_roller_set_sel(lv_obj_t *roller, struct layer_roller_state state)
{
    if (state.index >= ZMK_KEYMAP_LAYERS_LEN) return;
    int sel_layer = state.index;
    if (layer_select_id[sel_layer] == -1) return;
    ...
    if (display_pos == center_pos) {
        ...
        static const lv_palette_t before_center[] = {
            LV_PALETTE_DEEP_ORANGE, LV_PALETTE_ORANGE, LV_PALETTE_AMBER,
        };
        static const lv_palette_t after_center[] = {
            LV_PALETTE_LIGHT_GREEN, LV_PALETTE_GREEN, LV_PALETTE_TEAL,
        };
        if (rel_pos < 0) {
            int idx = (-rel_pos) - 1;
    ...
    /* Roller select API variations across LVGL builds */
...
#else
    /* Some LVGL configs expose roller via "lv_dropdown"-like API: try lv_obj_set_style_text_align as no-op fallback */
    (void)display_pos;
#endif
}
...
static void layer_roller_update_cb(struct layer_roller_state state)
{
    ...
    SYS_SLIST_FOR_EACH_CONTAINER(&widgets, widget, node) {
        if (widget && widget->obj) layer_roller_set_sel(widget->obj, state);
    }
}
...
static void mask_event_cb(lv_event_t *e)
{
    ...
    /* Keep mask ids local static to track add/remove across callbacks */
    ...
    if (code == LV_EVENT_COVER_CHECK) {
        ...
        /* LVGL v8 software mask API */
#if defined(lv_draw_sw_mask_fade_param_t) && defined(lv_draw_sw_mask_fade_init) && defined(lv_draw_sw_mask_add)
        {
...
#elif defined(lv_draw_mask_fade_param_t) && defined(lv_draw_mask_fade_init) && defined(lv_draw_mask_add)
        /* Older LVGL mask API */
        {
...
#else
        /* Masks not supported in detected LVGL headers: do nothing */
        (void)line_space;
#endif
        ...
        if (mask_top_id >= 0) {
            ...
            if (p) { lv_draw_sw_mask_free_param(p); lv_mem_free(p); }
            mask_top_id = -1;
        }
        if (mask_bottom_id >= 0) {
...
#elif defined(lv_draw_mask_remove_id) && defined(lv_draw_mask_free_param)
        if (mask_top_id >= 0) {
            lv_draw_mask_fade_param_t *p = lv_draw_mask_remove_id(mask_top_id);
            if (p) { lv_draw_mask_free_param(p); lv_mem_free(p); }
            mask_top_id = -1;
        }
        if (mask_bottom_id >= 0) {
...
static void init_layer_arrays(void)
{
    ...
    for (int i = 0; i < ZMK_KEYMAP_LAYERS_LEN; i++) {
        ...
        if (name != NULL) total_layers++;
    }
    if (total_layers <= 0) total_layers = 1;
...
int zmk_widget_layer_roller_init(struct zmk_widget_layer_roller *widget, lv_obj_t *parent)
{
...
#elif defined(lv_dropdown_create) /* fallback very unlikely but safe */
    widget->obj = (lv_obj_t *)lv_dropdown_create(parent);
#else
    widget->obj = NULL;
#endif
    ...
    for (int i = 0; i < total_layers; i++) {
        ...
        for (const char *c = layer_name; *c && rem > 1; c++) { *ptr++ = (char)toupper((unsigned char)*c); rem--; }
        if (rem > 0) { *ptr = '\0'; rem = sizeof(layer_names_buffer) - strlen(layer_names_buffer); }
#else
